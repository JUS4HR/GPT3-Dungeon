...
def _checkPasswd(uname: str, passwd: str):
...
def _checkAuth(uid: str, token: str):
    with open(jsonPath, "r") as f:
        data = _j.load(f)
        for user in data:
...
def _addAuth(uname: str, password: str):
    if not _o.path.exists(jsonPath):
    ...
    while udi in currentUidList or udi == 0:
        udi = _r.randint(1000000000, 9999999999)
    token = ""
...
def authTokenCallback(input: _CallableInputType) -> _CallbackReturnType:
    return _ResponseType.JSON, _checkAuth(input["uid"], input["token"])


def authPasswordCallback(input: _CallableInputType) -> _CallbackReturnType:
    return _ResponseType.JSON, _checkPasswd(input["uname"], input["pass"])
